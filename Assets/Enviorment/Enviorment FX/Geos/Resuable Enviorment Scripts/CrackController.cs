...
public class CrackController : MonoBehaviour
{
    ...
    [SerializeField] GroundCrackScript _CrackPrefab;//Gameobj input
    [SerializeField] float _OpenValue;//BlendshapeValue
    [SerializeField] float _Speed;//How fast
    ...
    List<GroundCrackScript> _cracks;//
    List<CrackController> crackControlls;
    ...
    public void Open(bool isSideCrack)
    {
    ...
    IEnumerator Coroutine_CrackOpen(bool isSideCrack) // will extend out crack to the square and instaciante the
                                                      // crack will instanshiate the transfrom foward over the parent
                                                      // and will seemlesslly blened
    {
        ...
        _cracks = new List<GroundCrackScript>();
        crackControlls = new List<CrackController>();

        int range = Mathf.RoundToInt(_range);
        Vector3 startPoint = transform.position;

        for (int i = 0; i < range; i += _CrackPrefab.lenght)
        {
            GroundCrackScript crack = Instantiate(_CrackPrefab, _Container);
            crack.transform.position = startPoint;
            crack.transform.forward = transform.forward;
            startPoint += transform.forward * _CrackPrefab.lenght * transform.localScale.z;

            //Can be comment out for testing position
        ...
        int rangeIndex = 0; //Controls how far the animation can go
        ...
        for (int i = 0; i < _cracks.Count; i++)
        {
            for (int j = 1; j < _cracks[i].BlendShapeCount; j++)
            {
                ...
                //Will Lerp through Animation Blendshapes Weird stuff happens 
                while (lerp < 1)
                {
                ...
                //if first blend shape is 100 then move to the next
                _cracks[i].SetBlendShape(j, _OpenValue);
                if (j == _cracks[i].BlendShapeCount - 1 && i < _cracks.Count - 1)
                {
                    _cracks[i + 1].SetBlendShape(0, _OpenValue);
                }
                //if  blend shape is 0 then move to the Open
            }
        }
        if (!isSideCrack)
        {
   ...
   */
    ...
    private void InstantiateSideCrack(int i, int j)
    {
        ...
        if (chance > _SideCrackChance)
        {
            ...
            CrackController crackControll = Instantiate(this, _Container);
            ...
            crackControll.Open(true);
            crackControlls.Add(crackControll);
        }
    }


    // Will lerp through blend shapes and close them as well as destroy
